...
import { TecnologyItem } from './TecItem'
import { tecnologies, actuallyStuding } from '../../utils/technologies'
    ...
    <div className="relative grow max-w-[1200px] z-10 pt-2">
      <Section>
        ...
        <div className="grid gap-2 grid-cols-2 md:grid-cols-4 max-w-[800px] mb-10">
          {tecnologies.map((tec) => (
            <TecnologyItem Icon={tec.icon} name={tec.name} key={tec.name} />
          ))}
        </div>
      </Section>
      <Section>
      ...
      <Section className="pb-20 pt-5">
        ...
        <Header
          headingLevel="1"
          text={'Confira mais em meu GitHub'}
          className="text-md md:text-md text-white-75a"
        />
        <MarkdownButton
          as="a"
